...
void main ()
{
	float dx = 1.0 / (textureSize(Sample0, 0)).x;
	// Locals
...
/*
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			texColour = texture(Sample0, uv + vec2(offset * dx * BlurScale, 0.0)) * Gaussian(offset * strength, deviation);
			colour += texColour;
		}
*/
	for (int i = 0; i < 10; i++) {
	    ...
	    float offset = float(i) - halfBlur;
	    colour += texture(Sample0, uv + vec2(PixOffset[i], 0.0) * dx * BlurScale ) * Gaussian(offset * strength, deviation); 
	    colour += texture(Sample0, uv - vec2(PixOffset[i], 0.0) * dx * BlurScale) * Gaussian(offset * strength, deviation); 
